...
sim_data_mCCAtdist = function(n = 200, nte = 1000, p = 200, s = 10, D = 5, seed =2021, ncomp = 5,
  ...
  Xagg = matrix(rt(n=n*ptotal, df =3), ncol = ptotal)%*%tmp$vectors%*%diag(sqrt(tmp$values))%*%t(tmp$vectors)
  Xtestagg = matrix(rt(n=nte*ptotal, df = 3), ncol = ptotal)%*%tmp$vectors%*%diag(sqrt(tmp$values))%*%t(tmp$vectors)
  Xagg = scale(Xagg)
...
p = 500; nte = 2000; ncomp = 3; alpha = 0; D = 4; ncomp1 = ncomp-1; nfolds = 10
path_name = "data/"
set.seed(2024)
for(n in c(300)){
  for(s in c(1, 5, 15)){
    for(type in c("identity", "toplitz", "spiked")){
      for(redundant in c(T, F)){
        print(paste0("Tdist_data_n", n, "_s",s,"_",type,"_redundant",redundant))
        for(iseed in c(1:50)){
          cont = F; t = 0
          save_file_name=paste0(path_name,"Tdist_data_n", n, "_s",s,"_",type,"_redundant",redundant,"_iseed",iseed, ".rds")
          while(! cont){
            seed = sample(1:100000,1)
            dat <- try(sim_data_mCCAtdist(n = n, nte = nte, p =p, s = s, D = D, seed =seed, ncomp = ncomp,redundant = redundant, type = type))
            t = t +1
            if(class(dat) != "try-error" | t > 3){
              cont = T
            }
          }
          if(class(dat) != "try-error"){
            foldid = sample(rep(1:nfolds, each = ceiling(n/nfolds)), n) 
            dat1 = list(dat = dat, foldid = foldid)
          }else{
            print("failed to generate the data!")
            dat1 = list(dat = NA, foldid = NA)
          }
          saveRDS(dat1 , file = save_file_name)
        }
      }
    }
  }
